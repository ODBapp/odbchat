...
OUT_DIR=$HOME/proj/odbchat/rag/json_out
# mkdir -p "$OUT_DIR"
find $HOME/proj/omnipipe/tests/rag/legacy_yamls -type f -name '*.yml' -print0 |
...
# test only one pdf
for f in $HOME/proj/omnipipe/tests/rag/raw_pdfs/woa23documentation.pdf; do
  base=$(basename "$f")
  curl -s -X POST -F "file=@$f" "http://localhost:${port}/v1/ingest" -F "pages=8,9,10,11,12" > "$OUT_DIR/${base%.*}.json"
done

# test only one img
for f in $HOME/proj/omnipipe/tests/rag/raw_images/odb_open_apis01.png; do
  base=$(basename "$f")
