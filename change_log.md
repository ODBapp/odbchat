...
#### v0.3.6 Move GHRSST to /mcp/metocean, and add tide.forecast mcp tool.
    ...
    - Router now prioritizes `tide.forecast` over GHRSST when a query mentions both tide and SST for the same lon/lat point, ensuring “潮高/海溫” combos don’t trigger the wrong tool (with regression tests covering the new behavior).
    - Classifier guidance now routes “如何使用 Hidy/odbchat_cli …” style questions to explain-mode instead of forcing code samples, so GUI usage questions again return step-by-step instructions drawn from the manuals.
